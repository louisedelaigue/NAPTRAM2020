...
day_list = so279_df['date_time'].dt.day.unique()

# create table to hold results per day
so279_stats = pd.DataFrame({'day':day_list})
...
for day in day_list:
    ...
    so279_stats.loc[so279_stats.day==day, "r2_sss"] = (stats.linregress(so279_df[L]['SBE45_sal'], so279_df[L]['pH_insitu'])[2])**2
    so279_stats.loc[so279_stats.day==day, "r2_sst"] = (stats.linregress(so279_df[L]['SBE38_water_temp'], so279_df[L]['pH_insitu'])[2])**2
    so279_stats.loc[so279_stats.day==day, "r2_ta"] = (stats.linregress(so279_df[L]['ta_est'], so279_df[L]['pH_insitu'])[2])**2
...
# compute R2 for overall cruise
R2_sss = (stats.linregress(so279_df['SBE45_sal'], so279_df['pH_insitu'])[2])**2
R2_sst = (stats.linregress(so279_df['SBE38_water_temp'], so279_df['pH_insitu'])[2])**2
R2_ta = (stats.linregress(so279_df['ta_est'], so279_df['pH_insitu'])[2])**2
