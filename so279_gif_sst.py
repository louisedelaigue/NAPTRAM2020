...
for ii in range(0, 37):
    ...
    satdat.thetao.isel(time=[ii]).plot(transform=ccrs.PlateCarree(),
                                    vmin=vmin,
                                    vmax=vmax,
                                    cmap=cmap,
                                    ax=ax)
    ...
    plt.savefig('./figs/gif_sst/{:02n}.png'.format(ii))
    plt.show()
    
# create gif
png_dir = './figs/gif_sst'
...
imageio.mimsave('./figs/gif_sst/gif_sst.gif', images, duration=0.5)
